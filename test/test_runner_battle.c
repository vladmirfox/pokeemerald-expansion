...
#define INVALID(fmt, ...) Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s:%d: " fmt, gTestRunnerState.test->filename, sourceLine, ##__VA_ARGS__)
#define INVALID_IF(c, fmt, ...) do { if (c) Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s:%d: " fmt, gTestRunnerState.test->filename, sourceLine, ##__VA_ARGS__); } while (0)

#define ASSUMPTION_FAIL_IF(c, fmt, ...) do { if (c) Test_ExitWithResult(TEST_RESULT_ASSUMPTION_FAIL, ":L%s:%d: " fmt, gTestRunnerState.test->filename, sourceLine, ##__VA_ARGS__); } while (0)
...
static void BattleTest_SetUp(void *data)
{
    ...
    if (STATE->parametersCount == 0 && test->resultsSize > 0)
        Test_ExitWithResult(TEST_RESULT_INVALID, ":Lresults without PARAMETRIZE");
    if (sizeof(*STATE) + test->resultsSize * STATE->parameters > sizeof(sBackupMapData))
        Test_ExitWithResult(TEST_RESULT_ERROR, ":LOOM: STATE (%d) + STATE->results (%d) too big for sBackupMapData (%d)", sizeof(*STATE), test->resultsSize * STATE->parameters, sizeof(sBackupMapData));
...
static void SetImplicitSpeeds(void)
{
    ...
    while (hasSpeeds != allSpeeds)
    {
        ...
        if (!madeProgress)
            Test_ExitWithResult(TEST_RESULT_INVALID, ":LTURNs have contradictory speeds");
    }
}
...
static void BattleTest_Run(void *data)
{
    ...
    if (DATA.playerPartySize < requiredPlayerPartySize)
        Test_ExitWithResult(TEST_RESULT_INVALID, ":L%d PLAYER Pokemon required", requiredPlayerPartySize);
    if (DATA.opponentPartySize < requiredOpponentPartySize)
        Test_ExitWithResult(TEST_RESULT_INVALID, ":L%d OPPONENT Pokemon required", requiredOpponentPartySize);
    ...
    if (DATA.hasExplicitSpeeds)
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_INVALID, ":LSpeed required for all PLAYERs and OPPONENTs");
        }
    }
    else
    {
...
u32 RandomUniform(enum RandomTag tag, u32 lo, u32 hi)
{
    ...
    if (tag == STATE->rngTag)
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_ERROR, ":LRandomUniform called with inconsistent trials %d and %d", STATE->trials, n);
        }
...
u32 RandomUniformExcept(enum RandomTag tag, u32 lo, u32 hi, bool32 (*reject)(u32))
{
    ...
    if (gCurrentTurnActionNumber < gBattlersCount)
    {
        ...
        if (turn && turn->rng.tag == tag)
        {
            if (reject(turn->rng.value))
                Test_ExitWithResult(TEST_RESULT_INVALID, ":LWITH_RNG specified a rejected value (%d)", turn->rng.value);
            return turn->rng.value;
        }
    }
    ...
    if (tag == STATE->rngTag)
    {
        ...
        while (reject(STATE->runTrial + lo + STATE->rngTrialOffset))
        {
            if (STATE->runTrial + lo + STATE->rngTrialOffset > hi)
                Test_ExitWithResult(TEST_RESULT_INVALID, ":LRandomUniformExcept called with inconsistent reject");
    ...
    {
        if (default_ == lo)
            Test_ExitWithResult(TEST_RESULT_INVALID, ":LRandomUniformExcept rejected all values");
        default_--;
    }
    return default_;
}
...
u32 RandomWeightedArray(enum RandomTag tag, u32 sum, u32 n, const u8 *weights)
{
    ...
    if (sum == 0)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":LRandomWeightedArray called with zero sum");
    ...
    if (tag == STATE->rngTag)
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_ERROR, ":LRandomWeighted called with inconsistent trials %d and %d", STATE->trials, n);
        }
    ...
    default:
        ...
        {
            if (n == 1)
                Test_ExitWithResult(TEST_RESULT_ERROR, ":LRandomWeightedArray called with all zero weights");
            n--;
        }
        return n-1;
    }
...
const void *RandomElementArray(enum RandomTag tag, const void *array, size_t size, size_t count)
{
    ...
    if (gCurrentTurnActionNumber < gBattlersCount)
    {
        ...
        if (turn && turn->rng.tag == tag)
        {
            ...
            Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s: RandomElement illegal value requested: %d", gTestRunnerState.test->filename, turn->rng.value);
        }
    }
    ...
    if (tag == STATE->rngTag)
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_ERROR, ":LRandomElement called with inconsistent trials %d and %d", STATE->trials, count);
        }
    ...
    case QUEUE_GROUP_NONE_OF:
        ...
        do
        {
            if ((match = TryAbilityPopUp(queuedEvent, event->groupSize, battlerId, ability)) != -1)
            {
                ...
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Matched ABILITY_POPUP", filename, line);
            }
    ...
    case QUEUE_GROUP_NONE_OF:
        ...
        do
        {
            if ((match = TryAnimation(queuedEvent, event->groupSize, animType, animId)) != -1)
            {
                const char *filename = gTestRunnerState.test->filename;
                u32 line = SourceLine(DATA.queuedEvents[match].sourceLineOffset);
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Matched ANIMATION", filename, line);
            }
    ...
    case QUEUE_GROUP_NONE_OF:
        ...
        do
        {
            if ((match = TryHP(queuedEvent, event->groupSize, battlerId, oldHP, newHP)) != -1)
            {
                const char *filename = gTestRunnerState.test->filename;
                u32 line = SourceLine(DATA.queuedEvents[match].sourceLineOffset);
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Matched HP_BAR", filename, line);
            }
...
void TestRunner_Battle_CheckChosenMove(u32 battlerId, u32 moveId, u32 target)
{
    ...
    if (!expectedAction->pass)
    {
        ...
        if (expectedAction->type != B_ACTION_USE_MOVE)
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Expected %s, got MOVE", filename, expectedAction->sourceLine, sBattleActionNames[expectedAction->type]);

        if (expectedAction->explicitTarget && expectedAction->target != target)
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Expected target %s, got %s", filename, expectedAction->sourceLine, BattlerIdentifier(expectedAction->target), BattlerIdentifier(target));
        ...
        if (!expectedAction->notMove && !movePasses)
        {
            ...
            if (countExpected > 1)
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched EXPECT_MOVES %S, got %S", filename, expectedAction->sourceLine, GetMoveName(expectedMoveId), GetMoveName(moveId));
            else
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched EXPECT_MOVE %S, got %S", filename, expectedAction->sourceLine, GetMoveName(expectedMoveId), GetMoveName(moveId));
        }
        if (expectedAction->notMove && !movePasses)
        {
            if (countExpected > 1)
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched NOT_EXPECT_MOVES %S", filename, expectedAction->sourceLine, GetMoveName(expectedMoveId));
            else
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched NOT_EXPECT_MOVE %S", filename, expectedAction->sourceLine, GetMoveName(expectedMoveId));
        }
    }
...
void TestRunner_Battle_CheckSwitch(u32 battlerId, u32 partyIndex)
{
    ...
    if (!expectedAction->pass)
    {
        if (expectedAction->type != B_ACTION_SWITCH)
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Expected %s, got SWITCH/SEND_OUT", filename, expectedAction->sourceLine, sBattleActionNames[expectedAction->type]);

        if (expectedAction->target != partyIndex)
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Expected partyIndex %d, got %d", filename, expectedAction->sourceLine, expectedAction->target, partyIndex);
    }
    DATA.aiActionsPlayed[battlerId]++;
}
...
{
    Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s: INVALID: %s trying to send out a mon(id: %d) with 0 HP.",
...
static void CheckIfMaxScoreEqualExpectMove(u32 battlerId, s32 target, struct ExpectedAIAction *aiAction, const char *filename)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: EXPECT_MOVE %S has the same best score(%d) as not expected MOVE %S", filename,
        ...
        if (scores[i] == scores[bestScoreId]
            ...
            && !(aiAction->moveSlots & gBitTable[bestScoreId]))
        {
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: NOT_EXPECT_MOVE %S has the same best score(%d) as MOVE %S", filename,
                                aiAction->sourceLine, GetMoveName(moves[i]), scores[i], GetMoveName(moves[bestScoreId]));
        }
    }
}
...
static void PrintAiMoveLog(u32 battlerId, u32 moveSlot, u32 moveId, s32 totalScore)
{
    ...
    {
        Test_ExitWithResult(TEST_RESULT_ERROR, ":LWarning! Score from logs(%d) is different than actual score(%d). Make sure all of the score adjustments use the ADJUST_SCORE macro\n", scoreFromLogs, totalScore);
    }
    MgbaPrintf_("Total: %d\n", totalScore);
}
    ...
    for (i = 0; i < MAX_AI_SCORE_COMPARISION_PER_TURN; i++)
    {
        ...
        if (scoreCtx->set)
        {
            ...
            if (scoreCtx->toValue)
            {
                ...
                {
                    Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched SCORE_%s_VAL %S %d, got %d",
            ...
            else
            {
                ...
                {
                    Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched SCORE_%s, got %S: %d, %S: %d",
    ...
    case QUEUE_GROUP_NONE_OF:
        ...
        do
        {
            if ((match = TryExp(queuedEvent, event->groupSize, battlerId, oldExp, newExp)) != -1)
            {
                const char *filename = gTestRunnerState.test->filename;
                u32 line = SourceLine(DATA.queuedEvents[match].sourceLineOffset);
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Matched EXPERIENCE_BAR", filename, line);
            }
    ...
    case QUEUE_GROUP_NONE_OF:
        ...
        do
        {
            if ((match = TryMessage(queuedEvent, event->groupSize, string)) != -1)
            {
                const char *filename = gTestRunnerState.test->filename;
                u32 line = SourceLine(DATA.queuedEvents[match].sourceLineOffset);
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Matched MESSAGE", filename, line);
            }
    ...
    case QUEUE_GROUP_NONE_OF:
        ...
        do
        {
            if ((match = TryStatus(queuedEvent, event->groupSize, battlerId, status1)) != -1)
            {
                const char *filename = gTestRunnerState.test->filename;
                u32 line = SourceLine(DATA.queuedEvents[match].sourceLineOffset);
                Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Matched STATUS_ICON", filename, line);
            }
...
void TestRunner_Battle_AfterLastTurn(void)
{
    ...
    {
        const char *filename = gTestRunnerState.test->filename;
        Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: %d TURNs specified, but %d ran", filename, SourceLine(0), DATA.turns, DATA.lastActionTurn + 1);
    }
    ...
    if (DATA.queuedEvent != DATA.queuedEventsCount)
    {
        ...
        Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: Unmatched %s", filename, line, macro);
    }
...
static void PushBattlerAction(u32 sourceLine, s32 battlerId, u32 actionType, u32 byte)
{
    ...
    if (recordIndex >= BATTLER_RECORD_SIZE)
        Test_ExitWithResult(TEST_RESULT_INVALID, ":LToo many actions");
     ...
     && DATA.battleRecordTypes[battlerId][recordIndex-1] != RECORDED_ACTION_TYPE)
    {
        ...
        for (i = recordIndex; i > 0; i--)
        {
             ...
             && DATA.recordedBattle.battleRecord[battlerId][i-1] == B_ACTION_USE_MOVE)
            {
                ...
                Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s:%d: Illegal MOVE", filename, line);
            }
        }
        Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s:%d: Illegal MOVE", filename, SourceLine(0));
    }
    ...
    if (DATA.battleRecordTypes[battlerId][recordIndex] != RECORDED_BYTE)
    {
        ...
        if (actionType != DATA.battleRecordTypes[battlerId][recordIndex])
        {
            ...
            if (actualMacro)
            {
                ...
                {
                case RECORDED_ACTION_TYPE:
                    Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s:%d: Expected MOVE/SWITCH, got %s", filename, line, actualMacro);
                case RECORDED_PARTY_INDEX:
                    Test_ExitWithResult(TEST_RESULT_INVALID, ":L%s:%d: Expected SEND_OUT, got %s", filename, line, actualMacro);
                }
            }

            Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: Illegal battle record", filename, line);
        }
    }
    else
    {
        ...
        {
            const char *filename = gTestRunnerState.test->filename;
            Test_ExitWithResult(TEST_RESULT_FAIL, ":L%s:%d: TURN %d incomplete", filename, SourceLine(0), gBattleResults.battleTurnCounter + 1);
        }
    }
}
...
void OpenTurn(u32 sourceLine)
{
    ...
    if (DATA.turns == MAX_TURNS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: TURN exceeds MAX_TURNS", gTestRunnerState.test->filename, sourceLine);
...
void QueueAbility(u32 sourceLine, struct BattlePokemon *battler, struct AbilityEventContext ctx)
{
    ...
    if (DATA.queuedEventsCount == MAX_QUEUED_EVENTS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: ABILITY exceeds MAX_QUEUED_EVENTS", gTestRunnerState.test->filename, sourceLine);
...
void QueueAnimation(u32 sourceLine, u32 type, u32 id, struct AnimationEventContext ctx)
{
    ...
    if (DATA.queuedEventsCount == MAX_QUEUED_EVENTS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: ANIMATION exceeds MAX_QUEUED_EVENTS", gTestRunnerState.test->filename, sourceLine);
...
void QueueHP(u32 sourceLine, struct BattlePokemon *battler, struct HPEventContext ctx)
{
    ...
    if (DATA.queuedEventsCount == MAX_QUEUED_EVENTS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: HP_BAR exceeds MAX_QUEUED_EVENTS", gTestRunnerState.test->filename, sourceLine);
...
void QueueExp(u32 sourceLine, struct BattlePokemon *battler, struct ExpEventContext ctx)
{
    ...
    if (DATA.queuedEventsCount == MAX_QUEUED_EVENTS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: EXPERIENCE_BAR exceeds MAX_QUEUED_EVENTS", gTestRunnerState.test->filename, sourceLine);
...
void QueueMessage(u32 sourceLine, const u8 *pattern)
{
    ...
    if (DATA.queuedEventsCount == MAX_QUEUED_EVENTS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: MESSAGE exceeds MAX_QUEUED_EVENTS", gTestRunnerState.test->filename, sourceLine);
    DATA.queuedEvents[DATA.queuedEventsCount++] = (struct QueuedEvent) {
...
void QueueStatus(u32 sourceLine, struct BattlePokemon *battler, struct StatusEventContext ctx)
{
    ...
    if (DATA.queuedEventsCount == MAX_QUEUED_EVENTS)
        Test_ExitWithResult(TEST_RESULT_ERROR, ":L%s:%d: STATUS_ICON exceeds MAX_QUEUED_EVENTS", gTestRunnerState.test->filename, sourceLine);
...
struct AILogLine *GetLogLine(u32 battlerId, u32 moveIndex)
{
    ...
    Test_ExitWithResult(TEST_RESULT_ERROR, ":LToo many AI log lines");
    return NULL;
}
