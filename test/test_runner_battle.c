...
static u32 CountAiExpectMoves(struct ExpectedAIAction *expectedAction, u32 battlerId, bool32 printLog)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            if (printLog)
                PrintAiMoveLog(battlerId, i, gBattleMons[battlerId].moves[i], gAiBattleStructData->aiFinalScore[battlerId][expectedAction->target][i]);
            countExpected++;
        }
    }
...
void TestRunner_Battle_CheckChosenMove(u32 battlerId, u32 moveId, u32 target)
{
    ...
    if (!expectedAction->pass)
    {
        ...
        if (!expectedAction->notMove && !movePasses)
        {
            ...
            PrintAiMoveLog(battlerId, moveSlot, moveId, gAiBattleStructData->aiFinalScore[battlerId][expectedAction->target][moveSlot]);
            if (countExpected > 1)
...
{
    u32 i;
    s32 *scores = gAiBattleStructData->aiFinalScore[battlerId][target];
