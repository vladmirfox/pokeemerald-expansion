...
DOUBLE_BATTLE_TEST("(DYNAMAX) Max Knuckle raises both allies' attack")
{
    ...
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_CLOSE_COMBAT].category == BATTLE_CATEGORY_PHYSICAL);
...
DOUBLE_BATTLE_TEST("(DYNAMAX) G-Max Snooze makes only the target drowsy")
{
    ...
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_DARK_PULSE].category == BATTLE_CATEGORY_SPECIAL); // Otherwise, Blissey faints.
...
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_DRAGON_CLAW].category == BATTLE_CATEGORY_PHYSICAL); // Otherwise Sableye faints.
