...
SINGLE_BATTLE_TEST("Parental Bond does not convert Triple Kick into a two-strike move")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Parental Bond converts multi-target moves into a two-strike move in Single Battles")
{
    GIVEN {
        ...
        TURN { MOVE(player, MOVE_EARTHQUAKE, megaEvolve: TRUE); MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
        MESSAGE("Kangaskhan's Kangaskhanite is reacting to 1's Mega Ring!");
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_MEGA_EVOLUTION, player);
        MESSAGE("Kangaskhan has Mega Evolved into Mega Kangaskhan!");
...
DOUBLE_BATTLE_TEST("Parental Bond does not convert multi-target moves into a two-strike move in Double Battles")
{
    u16 move;
    ...
    GIVEN {
        ...
        ASSUME(gBattleMoves[MOVE_EARTHQUAKE].strikeCount < 2);
        ASSUME(gBattleMoves[MOVE_EARTHQUAKE].effect == EFFECT_EARTHQUAKE);
        ASSUME(gBattleMoves[MOVE_ROCK_SLIDE].split != SPLIT_STATUS);
        ASSUME(gBattleMoves[MOVE_ROCK_SLIDE].strikeCount < 2);
        ASSUME(gBattleMoves[MOVE_ROCK_SLIDE].effect == EFFECT_FLINCH_HIT);
        PLAYER(SPECIES_KANGASKHAN) { Item(ITEM_KANGASKHANITE); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, move, megaEvolve: TRUE); MOVE(playerRight, MOVE_CELEBRATE); MOVE(opponentLeft, MOVE_CELEBRATE); MOVE(opponentRight, MOVE_CELEBRATE); }
    } SCENE {
        MESSAGE("Kangaskhan's Kangaskhanite is reacting to 1's Mega Ring!");
...
SINGLE_BATTLE_TEST("Parental Bond-converted moves only hit once on Lightning Rod/Storm Drain mons")
{
    ...
    GIVEN {
        ASSUME(gBattleMoves[move].split != SPLIT_STATUS);
