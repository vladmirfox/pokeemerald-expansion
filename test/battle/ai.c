...
AI_SINGLE_BATTLE_TEST("AI prefers moves with better accuracy, but only if they both require the same number of hits to ko")
{
    ...
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_SLAM].category == BATTLE_CATEGORY_PHYSICAL);
        ...
        ASSUME(gMovesInfo[MOVE_MEGA_KICK].category == BATTLE_CATEGORY_PHYSICAL);
        ASSUME(gMovesInfo[MOVE_SWIFT].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_SHOCK_WAVE].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_ICY_WIND].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_THUNDERBOLT].category == BATTLE_CATEGORY_SPECIAL);
...
AI_SINGLE_BATTLE_TEST("AI prefers moves which deal more damage instead of moves which are super-effective but deal less damage")
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_WATERFALL].category == BATTLE_CATEGORY_PHYSICAL);
        ASSUME(gMovesInfo[MOVE_SCALD].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_POISON_JAB].category == BATTLE_CATEGORY_PHYSICAL);
...
AI_SINGLE_BATTLE_TEST("AI prefers Earthquake over Drill Run if both require the same number of hits to ko")
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_EARTHQUAKE].category == BATTLE_CATEGORY_PHYSICAL); // Added because Geodude has to KO Typhlosion
        ASSUME(gMovesInfo[MOVE_DRILL_RUN].category == BATTLE_CATEGORY_PHYSICAL);  // Added because Geodude has to KO Typhlosion
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
...
AI_SINGLE_BATTLE_TEST("AI prefers a weaker move over a one with a downside effect if both require the same number of hits to ko")
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_FLAMETHROWER].category == BATTLE_CATEGORY_SPECIAL); // Added because Typhlosion has to KO Wobbuffet
        ASSUME(gMovesInfo[MOVE_OVERHEAT].category == BATTLE_CATEGORY_SPECIAL);     // Added because Typhlosion has to KO Wobbuffet
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_WOBBUFFET) { HP(hp); }
        PLAYER(SPECIES_WOBBUFFET);
...
AI_SINGLE_BATTLE_TEST("AI won't use Solar Beam if there is no Sun up or the user is not holding Power Herb")
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_SOLAR_BEAM].category == BATTLE_CATEGORY_SPECIAL);
...
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_EARTHQUAKE].category == BATTLE_CATEGORY_PHYSICAL); // Otherwise, it doesn't KO Crobat
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
...
AI_DOUBLE_BATTLE_TEST("AI will not use a status move if partner already chose Helping Hand")
{
    ...
    {
        if (gMovesInfo[j].category == BATTLE_CATEGORY_STATUS) {
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_MON_CHOICES: AI will not switch in a Pokemon which is slower and gets 1HKOed after fainting")
{
    ...
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_FOCUS_BLAST].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_BUBBLE_BEAM].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_WATER_GUN].category == BATTLE_CATEGORY_SPECIAL);
        ASSUME(gMovesInfo[MOVE_STRENGTH].category == BATTLE_CATEGORY_PHYSICAL);
