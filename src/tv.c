...
static u16 GetRandomDifferentSpeciesSeenByPlayer(u16 excludedSpecies)
{
    ...
    while (GetPokedexFlagFirstSeen(SpeciesToNationalPokedexNum(species)) == FALSE || species == excludedSpecies)
    {
...
{
    if (!GetPokedexFlagFirstSeen(SpeciesToNationalPokedexNum(species)) ? TRUE : FALSE)
        gSaveBlock1Ptr->tvShows[showIdx].common.active = FALSE;
}
