...
static void Cmd_switchineffects(void)
{
    ...
    // Healing Wish activates before hazards
        ...
        && gBattleStruct->storedHealingWish & gBitTable[GetBattlerPosition(gActiveBattler)])
        || (gBattleStruct->storedHealingWish & gBitTable[GetBattlerPosition(gActiveBattler)]
        && (gBattleMons[gActiveBattler].hp != gBattleMons[gActiveBattler].maxHP 
        || gBattleMons[gActiveBattler].status1 != 0)))
    {
        gBattleStruct->storedHealingWish &= ~(gBitTable[GetBattlerPosition(gActiveBattler)]);
        BattleScriptPushCursor();
        if (gBattleStruct->storedHealingWish & gBitTable[GetBattlerPosition(gActiveBattler) + MAX_BATTLERS_COUNT]) // Lunar Dance check
        {
            gBattlescriptCurrInstr = BattleScript_LunarDanceActivates;
            gBattleStruct->storedHealingWish &= ~(gBitTable[GetBattlerPosition(gActiveBattler) + MAX_BATTLERS_COUNT]);
        }
        else
        {
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_STORE_HEALING_WISH:
        // Bits 0-3 represent whether a battler position has a stored Healing Wish
        // Bits 4-7 represents this stored Healing Wish came from Lunar Dance
