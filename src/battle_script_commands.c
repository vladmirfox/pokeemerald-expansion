...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        case STATUS1_SLEEP:
            ...
            if (i != gBattlersCount)
                break;
            if (!CanBeSleptOrDrowsy(gEffectBattler, GetBattlerAbility(gEffectBattler), BLOCKED_BY_SLEEP_CLAUSE) && !(gBattleStruct->sleepClauseEffectExempt & (1u << gEffectBattler)))
                break;
    ...
    case VARIOUS_PSYCHO_SHIFT:
        {
            ...
            else if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP) && CanBeSleptOrDrowsy(gBattlerTarget, targetAbility, BLOCKED_BY_SLEEP_CLAUSE))
