                ...
                if (ejectButtonBattlers || ejectPackBattlers)
                {
                    ...
                    for (i = 0; i < gBattlersCount; i++)
                    {
                        ...
                        if (IsBattlerAlive(battler)
                            ...
                            && !(gBattleMoves[gCurrentMove].effect == EFFECT_HIT_SWITCH_TARGET && CanBattlerSwitch(gBattlerAttacker))
                            )
                        {
        ...
        case MOVEEND_RED_CARD:
            {
                ...
                if (redCardBattlers
                  && (gBattleMoves[gCurrentMove].effect != EFFECT_HIT_SWITCH_TARGET || gBattleStruct->hitSwitchTargetFailed)
                  && IsBattlerAlive(gBattlerAttacker)
                  && !TestSheerForceFlag(gBattlerAttacker, gCurrentMove)
