...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (!InBattlePike())
    {
        ...
        if (GetNumberOfRelearnableMoves(&mons[slotId]) != 0 && FlagGet(FLAG_RELEARN_AVAILABLE))
...
static void CB2_ReturnToPartyMenuWhileLearningMove(void)
{
    if ((gSpecialVar_ItemId == ITEM_RARE_CANDY || gSpecialVar_ItemId == ITEM_INFINITE_CANDY) && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (sInitialLevel < GetPkmnLevelCap())
    {
