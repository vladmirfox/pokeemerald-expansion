...
static void ShowMoveSelectWindow(u8 slot)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        AddTextPrinterParameterized(windowId, fontId, GetBattleMoveName(move), 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
        if (move != MOVE_NONE)
            moveCount++;
    }
...
void ItemUseCB_TMHM(u8 taskId, TaskFunc task)
{
    ...
    GetMonNickname(mon, gStringVar1);
    StringCopy(gStringVar2, GetBattleMoveName(move));
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    GetMonNickname(mon, gStringVar1);
    StringCopy(gStringVar2, GetBattleMoveName(move[0]));
...
{
    StringCopy(gStringVar2, GetBattleMoveName(gPartyMenu.data1));
...
{
    GetMonNickname(&gPlayerParty[gPartyMenu.slotId], gStringVar1);
    StringCopy(gStringVar2, GetBattleMoveName(gMoveToLearn));
