...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (!InBattlePike())
    {
        ...
        FlagGet(FLAG_DISABLE_MON_EDITING) == FALSE)
    ...
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_FLY) && CheckBagHasItem(ITEM_HM02, 1)) 
    AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_FLASH) && CheckBagHasItem(ITEM_HM05, 1)) 
...
static void ChangePokemonNicknamePartyScreen(void)
{
    ...
    ChangePokemonNicknamePartyScreen_CB);
}
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (!(B_RARE_CANDY_CAP && sInitialLevel >= GetCurrentLevelCap()) && sInitialLevel < gCurrentLevelCap())
    {
    ...
    if (cannotUseEffect)
    {
        ...
        {
            gPartyMenuUseExitCallback = FALSE;
            DisplayPartyMenuMessage(gText_ReachedLevelCap, TRUE);
            ScheduleBgCopyTilemapToVram(2);
            gTasks[taskId].func = task;
        }
    }
...
static UNUSED void Task_DisplayLevelUpStatsPg1(u8 taskId)
{
...
static bool8 IsMonNotFullyHealed(void)
{
    ...
    if (currentHP < maxHP && currentHP != 0)
        return TRUE;  // Found a Pokémon with less than full HP, but not fainted
