...
u16 GetCurrentMapWildMonHeaderId(void)
{
    ...
    for (i = 0; ; i++)
    {
            ...
            gWildMonHeaders[i].mapNum == gSaveBlock1Ptr->location.mapNum)
        {
			...
			if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(VERDANT_ISLE) &&
                gSaveBlock1Ptr->location.mapNum == MAP_NUM(VERDANT_ISLE))
                i += VarGet(VAR_DAYNIGHT);

			if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(VERDANT_ISLE_DEEP) &&
                gSaveBlock1Ptr->location.mapNum == MAP_NUM(VERDANT_ISLE_DEEP))
                i += VarGet(VAR_DAYNIGHT);
			...
			if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(LILYCOVE_CITY_LILYCOVE_MUSEUM_GARDEN) &&
                ...
                i += ((VarGet(VAR_TROPHY_GARDEN_ENCOUNTERS) * 2) + VarGet(VAR_DAYNIGHT));
