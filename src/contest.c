    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
            ...
            && eContestantStatus[gContestPlayerMonIndex].hasJudgesAttention)
        {
            ...
            moveNameBuffer = StringCopy(moveName, gText_ColorLightShadowDarkGray);
                 ...
                 && gBattleMoves[move].contestEffect != CONTEST_EFFECT_REPETITION_NOT_BORING)
        {
            // Gray the text because it's a repeated move
            moveNameBuffer = StringCopy(moveName, gText_ColorBlue);
        }
        moveNameBuffer = StringCopy(moveNameBuffer, GetBattleMoveName(move));
...
static void Task_DoAppeals(u8 taskId)
{
    ...
    case APPEALSTATE_PRINT_USED_MOVE_MSG:
        ...
        else
        {
            ...
            if (eContestantStatus[contestant].currMove < MOVES_COUNT)
                StringCopy(gStringVar2, GetBattleMoveName(eContestantStatus[contestant].currMove));
            else
    ...
    case APPEALSTATE_UPDATE_CROWD:
        ...
        else
        {
            ...
            {
                r3 = 1;
                StringCopy(gStringVar3, GetBattleMoveName(eContestantStatus[contestant].currMove));
            }
            else
            {
                StringCopy(gStringVar3, sContestConditions[gBattleMoves[eContestantStatus[contestant].currMove].contestCategory]);
            }
...
static u16 GetMoveEffectSymbolTileOffset(u16 move, u8 contestant)
{
    ...
    switch (gContestEffects[gBattleMoves[move].contestEffect].effectType)
    {
    case 0:
    case 1:
    case 8:
...
static void PrintContestMoveDescription(u16 move)
{
    ...
    category = gBattleMoves[move].contestCategory;
    ...
    // Appeal hearts
    if (gContestEffects[gBattleMoves[move].contestEffect].appeal == 0xFF)
        numHearts = 0;
    else
        numHearts = gContestEffects[gBattleMoves[move].contestEffect].appeal / 10;
    ...
    // Jam hearts
    if (gContestEffects[gBattleMoves[move].contestEffect].jam == 0xFF)
        numHearts = 0;
    else
        numHearts = gContestEffects[gBattleMoves[move].contestEffect].jam / 10;
    if (numHearts > MAX_CONTEST_MOVE_HEARTS)
        numHearts = MAX_CONTEST_MOVE_HEARTS;
    ...
    Contest_PrintTextToBg0WindowStd(WIN_MOVE_DESCRIPTION, gContestEffectDescriptionPointers[gBattleMoves[move].contestEffect]);
...
static void CalculateAppealMoveImpact(u8 contestant)
{
    ...
    effect = gBattleMoves[move].contestEffect;

    eContestantStatus[contestant].moveCategory = gBattleMoves[eContestantStatus[contestant].currMove].contestCategory;
    ...
    if (IsContestantAllowedToCombo(contestant))
    {
        ...
        {
            if (gBattleMoves[eContestantStatus[contestant].currMove].contestComboStarterId != 0)
            {
...
static void PrintAppealMoveResultText(u8 contestant, u8 stringId)
{
    StringCopy(gStringVar1, gContestMons[contestant].nickname);
    StringCopy(gStringVar2, GetBattleMoveName(eContestantStatus[contestant].currMove));
    if      (gBattleMoves[eContestantStatus[eContestAppealResults.contestant].currMove].contestCategory == CONTEST_CATEGORY_COOL)
        StringCopy(gStringVar3, gText_Contest_Shyness);
    else if (gBattleMoves[eContestantStatus[eContestAppealResults.contestant].currMove].contestCategory == CONTEST_CATEGORY_BEAUTY)
    ...
    else if (gBattleMoves[eContestantStatus[eContestAppealResults.contestant].currMove].contestCategory == CONTEST_CATEGORY_SMART)
...
{
    return sContestExcitementTable[gSpecialVar_ContestCategory][gBattleMoves[move].contestCategory];
}
