...
static void PutMovesPointsText(struct BattleDebugMenu *data)
{
    ...
    {
        text[0] = CHAR_SPACE;
        StringCopy(text + 1, GetBattleMoveName(gBattleMons[data->aiBattlerId].moves[i]));
...
static void PutAiPartyText(struct BattleDebugMenu *data)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        *txtPtr = EOS;
        ...
        {
            txtPtr = StringCopyN(text, GetBattleMoveName(aiMons[i].moves[j]), 8);
            *txtPtr = EOS;
...
static void PrintSecondaryEntries(struct BattleDebugMenu *data)
{
    ...
    case LIST_ITEM_PP:
        for (i = 0; i < 4; i++)
        {
            PadString(GetBattleMoveName(gBattleMons[data->battlerId].moves[i]), text);
