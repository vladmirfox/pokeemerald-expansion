...
static void GenerateInitialRentalMons(void)
{
    ...
    while (i != PARTY_SIZE)
    {
        ...
        {
            if (heldItems[j] != 0 && heldItems[j] == gFacilityTrainerMons[monSetId].itemId)
            {
        ...
        heldItems[i] = gFacilityTrainerMons[monSetId].itemId;
        monIds[i] = monSetId;
        i++;
    }
}
...
static void GenerateOpponentMons(void)
{
    ...
    while (i != FRONTIER_PARTY_SIZE)
    {
        ...
        for (k = 0; k < i; k++)
        {
            if (heldItems[k] != ITEM_NONE && heldItems[k] == gFacilityTrainerMons[sRandMonId].itemId)
                break;
        }
        if (k != i)
            continue;
        ...
        heldItems[i] = gFacilityTrainerMons[sRandMonId].itemId;
