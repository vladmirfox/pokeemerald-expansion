...
static void UpdateLvlInHealthbox(u8 healthboxSpriteId, u8 lvl)
{
    ...
    // Don't print Lv char if mon is mega evolved, primal reverted, Dynamaxed, or reverse mode.
    if (IsBattlerMegaEvolved(battler) || IsBattlerPrimalReverted(battler) || IsDynamaxed(battler) || reverse)
    {
...
static const struct SpritePalette sMegaIndicator_SpritePalettes[] =
{
    ...
    [INDICATOR_REVERSE] = {sAlphaOmegaIndicatorPal, TAG_MISC_INDICATOR_PAL},
    [INDICATOR_COUNT] = {0}
};
...
static const u16 sMegaIndicatorTags[][2] =
{
    ...
    [INDICATOR_REVERSE] = {TAG_REVERSE_INDICATOR_TILE, TAG_MISC_INDICATOR_PAL},
};
...
static bool32 MegaIndicator_ShouldBeInvisible(u32 battlerId, struct Sprite *sprite)
{
    ...
    if (!megaEvolved && !primalReverted && !dynamaxed && !reverseMode)
        return TRUE;
