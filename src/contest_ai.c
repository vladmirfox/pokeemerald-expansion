...
static void ContestAICmd_get_move_effect(void)
{
    ...
    eContestAI.scriptResult = gBattleMoves[move].contestEffect;
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_get_move_effect_type(void)
{
    ...
    eContestAI.scriptResult = gContestEffects[gBattleMoves[move].contestEffect].effectType;
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_check_most_appealing_move(void)
{
    int i;
    u16 move = gContestMons[eContestAI.contestantId].moves[eContestAI.nextMoveIndex];
    u8 appeal = gContestEffects[gBattleMoves[move].contestEffect].appeal;

    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        u16 newMove = gContestMons[eContestAI.contestantId].moves[i];
        if (newMove != 0 && appeal < gContestEffects[gBattleMoves[newMove].contestEffect].appeal)
            break;
    }
...
static void ContestAICmd_check_most_jamming_move(void)
{
    int i;
    u16 move = gContestMons[eContestAI.contestantId].moves[eContestAI.nextMoveIndex];
    u8 jam = gContestEffects[gBattleMoves[move].contestEffect].jam;

    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        u16 newMove = gContestMons[eContestAI.contestantId].moves[i];
        if (newMove != MOVE_NONE && jam < gContestEffects[gBattleMoves[newMove].contestEffect].jam)
            break;
    }
...
static void ContestAICmd_get_num_move_hearts(void)
{
    ...
    eContestAI.scriptResult = gContestEffects[gBattleMoves[move].contestEffect].appeal / 10;
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_get_num_move_jam_hearts(void)
{
    ...
    eContestAI.scriptResult = gContestEffects[gBattleMoves[move].contestEffect].jam / 10;
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_get_used_combo_starter(void)
{
    ...
    if (IsContestantAllowedToCombo(contestant))
        result = gBattleMoves[eContestantStatus[contestant].prevMove].contestComboStarterId ? TRUE : FALSE;
...
static void ContestAICmd_check_user_has_move(void)
{
    ...
    {
        #ifdef BUGFIX
        u16 move = gBattleMoves[gContestMons[eContestAI.contestantId].moves[i]].contestEffect;
        #else
