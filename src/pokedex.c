...
struct PokedexView
{
    struct PokedexListItem pokedexList[NUM_SPECIES + 1];
...
static void ResetPokedexView(struct PokedexView *pokedexView)
{
    ...
    pokedexView->pokedexList[NUM_SPECIES].dexNum = 0;
    pokedexView->pokedexList[NUM_SPECIES].seenSpecies = SPECIES_NONE;
    pokedexView->pokedexList[NUM_SPECIES].owned = FALSE;
    ...
    case ORDER_NUMERICAL:
        {
            ...
            for (i = 0, listCount = 0; i < temp_dexCount; i++)
            {
                if (temp_isHoennDex)
            ...
            #if P_DEX_EMPTY_ENTRY_SKIP == FALSE
                listCount = i;
            #else
                listCount = sPokedexView->pokemonListCount;
                if (seenSpecies != SPECIES_NONE)
                {
                    ...
                    #if P_DEX_EMPTY_ENTRY_AT_ENDS == FALSE
                        && listCount != 0
                    #endif
                    )
                    {
                        if (temp_isHoennDex)
                            sPokedexView->pokedexList[listCount].dexNum = HoennToNationalOrder(i);
                        else
                            sPokedexView->pokedexList[listCount].dexNum = i;
                        sPokedexView->pokedexList[listCount].seenSpecies = SPECIES_NONE;
                        sPokedexView->pokedexList[listCount].owned = FALSE;
                        listCount++;
                    }
                }
            ...
            #endif
                sPokedexView->pokedexList[listCount].dexNum = temp_dexNum;
                sPokedexView->pokedexList[listCount].seenSpecies = seenSpecies;
                sPokedexView->pokedexList[listCount].owned = GetSetPokedexFlag(seenSpecies, FLAG_GET_CAUGHT);
                if (seenSpecies != SPECIES_NONE || previousWasSeen)
                    sPokedexView->pokemonListCount = listCount + 1;
            #if P_DEX_EMPTY_ENTRY_SKIP == TRUE
                secondPreviousSeen = previousWasSeen;
                previousWasSeen = seenSpecies != SPECIES_NONE;
            #endif
            }
        }
        break;
    ...
    case ORDER_HEAVIEST:
        for (i = ARRAY_COUNT(gPokedexOrder_Weight) - 1; i >= 0; i--)
        {
            ...
            if ((!temp_isHoennDex || NationalToHoennOrder(temp_dexNum) != 0)
                ...
                && GetSetPokedexFlag(species, FLAG_GET_SEEN))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = temp_dexNum;
...
static void CreateMonListEntry(u8 position, u16 b, u16 ignored)
{
    ...
    default:
        ...
        for (i = 0; i <= 10; i++)
        {
            if (entryNum < 0 || entryNum >= NUM_SPECIES || sPokedexView->pokedexList[entryNum].dexNum == 0xFFFF)
            {
...
{
    if (species >= NUM_SPECIES || sPokedexView->pokedexList[species].dexNum == 0xFFFF)
        return 0xFFFF;
...
u8 DisplayCaughtMonDexPage(u16 species, u32 otId, u32 personality)
{
    ...
    gTasks[taskId].tDexNum = SpeciesToNationalPokedexNum(species);
...
u16 GetPokedexFlagFirstSeen(u16 nationalDexNo)
{
    ...
    if (formTable != NULL)
    {
        ...
        {
            if (GetSetPokedexFlag(formTable[formId], FLAG_GET_SEEN))
                return formTable[formId];
        }
    }
    else if (GetSetPokedexFlag(NationalPokedexNumToSpecies(nationalDexNo), FLAG_GET_SEEN))
    {
        return NationalPokedexNumToSpecies(nationalDexNo);
    }
    return SPECIES_NONE;
}
...
static void HandleFormSwitch_ScrollingList(u16 selectedMon, bool8 nextForm)
{
    ...
    while (TRUE)
    {
        ...
        if (newSpecies == sPokedexView->pokedexList[selectedMon].seenSpecies)
            break;
        if (GetSetPokedexFlag(newSpecies, FLAG_GET_SEEN))
        {
            found = TRUE;
            break;
        }
    ...
    if (found)
    {
        ...
        sPokedexView->pokedexList[selectedMon].owned = GetSetPokedexFlag(newSpecies, FLAG_GET_CAUGHT);
        ClearMonSprites();
        CreateMonSpritesAtPos(selectedMon, 0xE);
