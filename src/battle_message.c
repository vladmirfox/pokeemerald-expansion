...
void BufferStringBattle(u16 stringID, u32 battler)
{
    ...
    case STRINGID_INTROMSG: // first battle msg
        ...
        else
        {
            ...
            else if (BATTLE_TYPE_IS_DOUBLE && IsValidForBattle(&gEnemyParty[gBattlerPartyIndexes[GetBattlerAtPosition(B_POSITION_OPPONENT_RIGHT)]]))
    ...
    case STRINGID_INTROSENDOUT: // poke first send-out
        ...
        {
            if (BATTLE_TYPE_IS_DOUBLE && IsValidForBattle(&gPlayerParty[gBattlerPartyIndexes[BATTLE_PARTNER(battler)]]))
            {
        ...
        {
            if (BATTLE_TYPE_IS_DOUBLE && IsValidForBattle(&gEnemyParty[gBattlerPartyIndexes[BATTLE_PARTNER(battler)]]))
            {
                if (BATTLE_TWO_VS_ONE_OPPONENT)
    ...
    case STRINGID_RETURNMON: // sending poke to ball msg
        if (GetBattlerSide(battler) == B_SIDE_PLAYER)
        {
            ...
            else if (*(&gBattleStruct->hpScale) == 1 || BATTLE_TYPE_IS_DOUBLE)
    ...
    case STRINGID_SWITCHINMON: // switch-in msg
        ...
        {
            if (*(&gBattleStruct->hpScale) == 0 || BATTLE_TYPE_IS_DOUBLE)
