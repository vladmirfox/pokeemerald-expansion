...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_CURSE:
        if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GHOST))
        {
            if (!AI_CanBattlerEscape(battlerDef, TRUE) && IsBattlerTrapped(battlerAtk, battlerDef))
                ADJUST_SCORE(GOOD_EFFECT);
            else
                ADJUST_SCORE(WEAK_EFFECT);
        }
...
static s32 AI_Roaming(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    if (!AI_CanBattlerEscape(battlerAtk, FALSE) && IsBattlerTrapped(battlerDef, battlerAtk))
        return score;
