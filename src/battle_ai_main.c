...
#define SET_SCORE(abs) \
    do \
    { \
        u32 _score = (abs); \
        TestRunner_Battle_AISetScore(__FILE__, __LINE__, sBattler_AI, AI_THINKING_STRUCT->movesetIndex, _score); \
        score = score; \
    } while (0)

#define ADJUST_SCORE(adj) \
    do \
    { \
        s32 _score = (adj); \
        TestRunner_Battle_AIAdjustScore(__FILE__, __LINE__, sBattler_AI, AI_THINKING_STRUCT->movesetIndex, _score); \
        score += _score; \
    } while (0)
...
void BattleAI_SetupAIData(u8 defaultScoreMoves)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            TestRunner_Battle_AISetScore(__FILE__, __LINE__, gActiveBattler, i, 100);
            AI_THINKING_STRUCT->score[i] = 100;
        }
        else
        {
            TestRunner_Battle_AISetScore(__FILE__, __LINE__, gActiveBattler, i, 0);
    ...
    // other
        ...
        case EFFECT_FAKE_OUT:
            ...
            else if (move == MOVE_FAKE_OUT) // filter out first impression
            {
                ...
                {
                    if (CountUsablePartyMons(battlerAtk) == 0)
                        ADJUST_SCORE(-10); // Don't lock the attacker into Fake Out if they can't switch out afterwards.
                }
            }
            break;
...
static s16 AI_TryToFaint(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    if (CanIndexMoveFaintTarget(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex, 0) && gBattleMoves[move].effect != EFFECT_EXPLOSION)
    {
        ...
        if (WillAIStrikeFirst() || GetMovePriority(battlerAtk, move) > 0)
            ADJUST_SCORE(+4); // we go first or we're using priority move
        else
            ADJUST_SCORE(+2);
    }
    else
    {
        ...
        if (TestMoveFlags(move, FLAG_HIGH_CRIT))
            ADJUST_SCORE(+2); // crit makes it more likely to make them faint
...
static s16 AI_CheckViability(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    if ((gBattleMons[battlerAtk].status1 & (STATUS1_FREEZE | STATUS1_FROSTBITE)) && TestMoveFlags(move, FLAG_THAW_USER))
        ADJUST_SCORE((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) ? +20 : +10);
        ...
        case ABILITY_NATURAL_CURE:
            if (AI_THINKING_STRUCT->aiFlags & AI_FLAG_SMART_SWITCHING
             && HasOnlyMovesWithSplit(battlerAtk, SPLIT_SPECIAL, TRUE))
                SET_SCORE(90); // Force switch if all your attacking moves are special and you have Natural Cure.
            break;
        default:
...
// stat lowering effects
    ...
    case EFFECT_CLEAR_SMOG:
        if (isDoubleBattle)
            ADJUST_SCORE(+min(CountPositiveStatStages(battlerDef) + CountPositiveStatStages(BATTLE_PARTNER(battlerDef)), 7));
        else
            ADJUST_SCORE(+min(CountPositiveStatStages(battlerDef), 4));
        break;
    ...
    case EFFECT_FLINCH_HIT:
        ADJUST_SCORE(ShouldTryToFlinch(battlerAtk, battlerDef, AI_DATA->abilities[battlerAtk], AI_DATA->abilities[battlerDef], move));
        break;
    case EFFECT_SWALLOW:
        if (gDisableStructs[battlerAtk].stockpileCounter == 0)
        {
    ...
    case EFFECT_CURSE:
        ...
        else
        {
            if (AI_DATA->abilities[battlerAtk] == ABILITY_CONTRARY || AI_DATA->abilities[battlerDef] == ABILITY_MAGIC_GUARD)
                ...
                ADJUST_SCORE(+(8 - gBattleMons[battlerAtk].statStages[STAT_ATK]));
            else if (gBattleMons[battlerAtk].statStages[STAT_SPEED] < 3)
                break;
            else if (gBattleMons[battlerAtk].statStages[STAT_DEF] < 8)
                ADJUST_SCORE(+(8 - gBattleMons[battlerAtk].statStages[STAT_DEF]));
        }
        break;
    case EFFECT_PROTECT:
    ...
    case EFFECT_BELLY_DRUM:
        if (!CanTargetFaintAi(battlerDef, battlerAtk) && HasMoveWithSplit(battlerAtk, SPLIT_PHYSICAL) && AI_DATA->abilities[battlerAtk] != ABILITY_CONTRARY)
            ADJUST_SCORE(+(MAX_STAT_STAGE - gBattleMons[battlerAtk].statStages[STAT_ATK]));
        break;
