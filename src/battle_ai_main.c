...
static s32 AI_DoubleBattle(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    case ABILITY_FLOWER_VEIL:
        if ((IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)) && (IsNonVolatileStatusMoveEffect(effect) || IsStatLoweringEffect(effect)))
            RETURN_SCORE(BAD_MOVE);
        break;
    case ABILITY_AROMA_VEIL:
        if (IsAromaVeilProtectedMove(move))
            RETURN_SCORE(BAD_MOVE);
        break;
    case ABILITY_DAZZLING: // Check if this is correct
    case ABILITY_QUEENLY_MAJESTY: // See above
    case ABILITY_ARMOR_TAIL: // See above
...
static u32 AI_CalcMoveScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_SMART_SWITCHING && AI_DATA->abilities[battlerAtk] == ABILITY_NATURAL_CURE)
    {
        ...
        || (gBattleMons[battlerAtk].status1 & STATUS1_FROSTBITE && HasOnlyMovesWithCategory(battlerAtk, DAMAGE_CATEGORY_SPECIAL, TRUE)))
            RETURN_SCORE(BAD_MOVE); // Force switch if all your attacking moves are physical and you have Natural Cure.
    }
    ...
    // check non-user target ability if possible
    ...
    if (AI_IsTerrainAffected(battlerDef, STATUS_FIELD_MISTY_TERRAIN)
     && (IsNonVolatileStatusMoveEffect(moveEffect) || IsConfusionMoveEffect(moveEffect)))
        RETURN_SCORE(BAD_MOVE);

    // if (AI_IsTerrainAffected(battlerAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && atkPriority > 0)
    //     RETURN_SCORE_MINUS(BAD_MOVE);
    ...
    case EFFECT_SLEEP:
    case EFFECT_YAWN:
        if (gStatuses3[battlerDef] & STATUS3_YAWN || AI_IsTerrainAffected(battlerDef, STATUS_FIELD_ELECTRIC_TERRAIN))
            ADJUST_SCORE(BAD_MOVE);
        else
    ...
    case EFFECT_MEMENTO:
        if (CountUsablePartyMons(battlerAtk) == 0
         ...
         || (gBattleMons[battlerDef].statStages[STAT_ATK] == MIN_STAT_STAGE && gBattleMons[battlerDef].statStages[STAT_SPATK] == MIN_STAT_STAGE))
            ADJUST_SCORE(BAD_MOVE);
        else if (AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_WILL_SUICIDE
              && gBattleMons[battlerDef].statStages[STAT_EVASION] < 7
    ...
    case EFFECT_STUFF_CHEEKS:
        if (ItemId_GetPocket(gBattleMons[battlerAtk].item) != POCKET_BERRIES)
            RETURN_SCORE(INVALID_MOVE);   // cannot even select
        IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF_2, &score);
        break;
    ...
    case EFFECT_HIT_SWITCH_TARGET:
        if (DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove)
         || (aiData->hpPercents[battlerDef] < 10 && GetBattlerSecondaryDamage(battlerDef))
         || gStatuses3[battlerDef] & STATUS3_PERISH_SONG)
            ADJUST_SCORE(BAD_MOVE);
        break;
    ...
    case EFFECT_ATTACK_SPATK_UP:    // work up
        ...
        break;
    case EFFECT_HAZE:
        if (AnyStatIsRaised(battlerAtk) || AnyStatIsRaised(BATTLE_PARTNER(battlerAtk))
          || PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else
    ...
    case EFFECT_CONVERSION:
        ...
        break;
    case EFFECT_SWALLOW:
        if (gDisableStructs[battlerAtk].stockpileCounter == 0 || aiData->hpPercents[battlerAtk] >= 80)
        {
            ADJUST_SCORE(BAD_MOVE);
            break;
        }
    ...
    case EFFECT_LASER_FOCUS:
        if (gBattleMons[battlerAtk].status2 & STATUS2_FOCUS_ENERGY_ANY
         || aiData->abilities[battlerDef] == ABILITY_SHELL_ARMOR || aiData->abilities[battlerDef] == ABILITY_BATTLE_ARMOR)
            ADJUST_SCORE(BAD_MOVE);
        else if (aiData->abilities[battlerAtk] == ABILITY_SUPER_LUCK
    ...
    case EFFECT_SUBSTITUTE:
        AdjustSubstituteScore(battlerAtk, battlerDef, move, &score);
        break;
    case EFFECT_MIMIC:
    ...
    case EFFECT_LEECH_SEED:
        if (AI_ShouldSetUpLeechSeed(battlerAtk, battlerDef, aiData))
        {
            ADJUST_SCORE(GOOD_EFFECT);
    ...
    case EFFECT_BATON_PASS:
        if (CountUsablePartyMons(battlerAtk) == 0)
            ADJUST_SCORE(BAD_MOVE);
        else if (ShouldSwitch(battlerAtk, FALSE) && (gBattleMons[battlerAtk].status2 & STATUS2_SUBSTITUTE
    ...
    case EFFECT_DISABLE:
        if (PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove)
            || AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_SLOWER
            || gLastMoves[battlerDef] == MOVE_NONE || gLastMoves[battlerDef] == 0xFFFF)
            ADJUST_SCORE(BAD_MOVE);
        else if (gDisableStructs[battlerDef].disableTimer == 0
              && (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER)
              && CanTargetMoveFaintAi(gLastMoves[battlerDef], battlerDef, battlerAtk, 1)
              && (gLastMoves[battlerDef] != MOVE_NONE)
              && (gLastMoves[battlerDef] != 0xFFFF)
              && (B_MENTAL_HERB < GEN_5 || aiData->holdEffects[battlerDef] != HOLD_EFFECT_MENTAL_HERB))
            ADJUST_SCORE(GOOD_EFFECT); // Disable move before attacker can faint
        break;
    case EFFECT_ENCORE:
        if (gDisableStructs[battlerDef].encoreTimer == 0
        && (B_MENTAL_HERB < GEN_5 || aiData->holdEffects[battlerDef] != HOLD_EFFECT_MENTAL_HERB)
    ...
    case EFFECT_LOCK_ON:
        if (gStatuses3[battlerDef] & STATUS3_ALWAYS_HITS
         || aiData->abilities[battlerAtk] == ABILITY_NO_GUARD || aiData->abilities[battlerDef] == ABILITY_NO_GUARD
         || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (HasMoveEffect(battlerAtk, EFFECT_OHKO))
            ADJUST_SCORE(GOOD_EFFECT);
    ...
    case EFFECT_HEAL_BELL:
        if (!AnyPartyMemberStatused(battlerAtk, gMovesInfo[move].soundMove) || PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (ShouldUseWishAromatherapy(battlerAtk, battlerDef, move))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_CURSE:
        if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GHOST))
    ...
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY
         || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE
    ...
    case EFFECT_BELLY_DRUM:
        if (aiData->abilities[battlerAtk] == ABILITY_CONTRARY || aiData->hpPercents[battlerAtk] <= 60)
            ADJUST_SCORE(BAD_MOVE);
        else if (!CanTargetFaintAi(battlerDef, battlerAtk)
    ...
    case EFFECT_FIRST_TURN_ONLY:
        ...
        if (ShouldFakeOut(battlerAtk, battlerDef, move))
            ADJUST_SCORE(GOOD_EFFECT);
        else if (gMovesInfo[move].argument == MOVE_FIRST_IMPRESSION && GetBestDmgMoveFromBattler(battlerAtk, battlerDef) == move)
            ADJUST_SCORE(BEST_EFFECT);
        break;
    case EFFECT_STOCKPILE:
        if (aiData->abilities[battlerAtk] == ABILITY_CONTRARY || gDisableStructs[battlerAtk].stockpileCounter >= 3)
            RETURN_SCORE(BAD_MOVE);
    ...
    case EFFECT_SAFEGUARD:
        if (gSideStatuses[GetBattlerSide(battlerAtk)] & SIDE_STATUS_SAFEGUARD
         || PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), moveEffect, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
    ...
    case EFFECT_MAGIC_COAT:
        if (!HasMagicCoatAffectedMove(battlerDef))
            ADJUST_SCORE(BAD_MOVE);
        else if (IS_MOVE_STATUS(predictedMove) && AI_GetBattlerMoveTargetType(battlerDef, predictedMove) & (MOVE_TARGET_SELECTED | MOVE_TARGET_OPPONENTS_FIELD | MOVE_TARGET_BOTH))
            ADJUST_SCORE(GOOD_EFFECT);
        break;
    case EFFECT_RECYCLE:
    ...
    case EFFECT_IMPRISON:
        if (gStatuses3[battlerAtk] & STATUS3_IMPRISONED_OTHERS)
            ADJUST_SCORE(BAD_MOVE);
        else if (predictedMove != MOVE_NONE && HasMove(battlerAtk, predictedMove))
            ADJUST_SCORE(DECENT_EFFECT);
    ...
    case EFFECT_MUD_SPORT:
        if (gFieldStatuses & STATUS_FIELD_MUDSPORT || gStatuses4[battlerAtk] & STATUS4_MUD_SPORT
         || PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (!HasMoveWithType(battlerAtk, TYPE_ELECTRIC) && HasMoveWithType(battlerDef, TYPE_ELECTRIC))
            ADJUST_SCORE(WEAK_EFFECT);
        break;
    case EFFECT_WATER_SPORT:
        if (gFieldStatuses & STATUS_FIELD_WATERSPORT || gStatuses4[battlerAtk] & STATUS4_WATER_SPORT
         || PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (!HasMoveWithType(battlerAtk, TYPE_FIRE) && (HasMoveWithType(battlerDef, TYPE_FIRE)))
            ADJUST_SCORE(WEAK_EFFECT);
        break;
    case EFFECT_TICKLE:
    ...
    case EFFECT_GEAR_UP:
    {
        ...
        if (isDoubleBattle
         && (aiData->abilities[battlerAtkPartner] == ABILITY_PLUS || aiData->abilities[battlerAtkPartner] == ABILITY_MINUS))
        {
            IncreaseStatUpScore(battlerAtkPartner, battlerDef, STAT_CHANGE_ATK, &score);
            IncreaseStatUpScore(battlerAtkPartner, battlerDef, STAT_CHANGE_SPATK, &score);
        }
        break;
    }
    ...
    case EFFECT_SPEED_SWAP:
        if (gBattleMons[battlerDef].speed > gBattleMons[battlerAtk].speed && !(gFieldStatuses & STATUS_FIELD_TRICK_ROOM))
            ADJUST_SCORE(DECENT_EFFECT);
        else
            ADJUST_SCORE(BAD_MOVE);
        break;
    ...
    case EFFECT_PLEDGE:
        ...
        {
            if (!((aiData->partnerMove != MOVE_NONE && move != aiData->partnerMove
             && gMovesInfo[aiData->partnerMove].effect == EFFECT_PLEDGE)
             || gBattleMons[BATTLE_PARTNER(battlerAtk)].status1 & (STATUS1_SLEEP | STATUS1_FREEZE)))
                ADJUST_SCORE(GOOD_EFFECT);
        }
        break;
    ...
    case EFFECT_WONDER_ROOM:
        if (gFieldStatuses & STATUS_FIELD_WONDER_ROOM || PartnerMoveIsSameNoTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if ((HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL) && gBattleMons[battlerAtk].defense < gBattleMons[battlerAtk].spDefense)
    ...
    case EFFECT_GRAVITY:
        ...
        {
            if ((!IS_BATTLER_OF_TYPE(battlerAtk, TYPE_FLYING) && aiData->holdEffects[battlerAtk] != HOLD_EFFECT_AIR_BALLOON) // Should revert Gravity in this case
    ...
    case EFFECT_ION_DELUGE:
        if (gFieldStatuses & STATUS_FIELD_ION_DELUGE
            || PartnerMoveIsSameNoTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if ((aiData->abilities[battlerAtk] == ABILITY_VOLT_ABSORB
    ...
    case EFFECT_EMBARGO:
        if (aiData->abilities[battlerDef] == ABILITY_KLUTZ
         ...
         || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (aiData->holdEffects[battlerDef] != HOLD_EFFECT_NONE)
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_POWDER:
//         if (PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
// PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove)
//         else if (predictedMove != MOVE_NONE && !IS_MOVE_STATUS(predictedMove) && gMovesInfo[predictedMove].type == TYPE_FIRE)
//             ADJUST_SCORE(DECENT_EFFECT);
//         break;
    case EFFECT_TELEKINESIS:
        if (IsTelekinesisBannedSpecies(gBattleMons[battlerDef].species) || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, FALSE, aiData->abilities[battlerAtk], aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk], aiData->holdEffects[battlerDef])
    ...
    case EFFECT_HEAL_BLOCK:
        if (gDisableStructs[battlerDef].healBlockTimer != 0
         || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
            ADJUST_SCORE(BAD_MOVE);
        else if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER && predictedMove != MOVE_NONE && IsHealingMove(predictedMove))
    ...
    case EFFECT_TAILWIND:
        if (gSideTimers[GetBattlerSide(battlerAtk)].tailwindTimer != 0
         ...
         || (gFieldStatuses & STATUS_FIELD_TRICK_ROOM && gFieldTimers.trickRoomTimer > 1)) // Trick Room active and not ending this turn
            ADJUST_SCORE(BAD_MOVE);
        else if (GetBattlerSideSpeedAverage(battlerAtk) < GetBattlerSideSpeedAverage(battlerDef))
            ADJUST_SCORE(GOOD_EFFECT);
        break;
    ...
    case EFFECT_MAGNET_RISE:
        ...
        else if (HasDamagingMoveOfType(battlerDef, TYPE_ELECTRIC) && !(AI_GetTypeEffectiveness(MOVE_EARTHQUAKE, battlerDef, battlerAtk) == AI_EFFECTIVENESS_x0)) // Doesn't resist ground move
        {
    ...
    case EFFECT_CAMOUFLAGE:
        if (!CanCamouflage(battlerAtk))
            ADJUST_SCORE(BAD_MOVE);
        else if (predictedMove != MOVE_NONE && AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER // Attacker goes first
    ...
    case EFFECT_SHORE_UP:
        if ((AI_GetWeather(aiData) & B_WEATHER_SANDSTORM) && ShouldRecover(battlerAtk, battlerDef, move, 67))
            ...
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_FOCUS_PUNCH: // Not really useful since it is already a damaging move and will be handled in CompareDamagingMoves (move check to IsMovesMinus)
    ...
    case EFFECT_JUNGLE_HEALING:
        if (AtMaxHp(battlerAtk)
         ...
         && !(gBattleMons[BATTLE_PARTNER(battlerAtk)].status1 & STATUS1_ANY))
            ADJUST_SCORE(BAD_MOVE);
        else if (ShouldRecover(battlerAtk, battlerDef, move, 25)
...
static s32 AI_PowerfulStatus(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    case EFFECT_STICKY_WEB:
    case EFFECT_TOXIC_SPIKES:
        if (AI_ShouldSetUpHazards(battlerAtk, battlerDef, move, AI_DATA))
            ADJUST_SCORE(POWERFUL_STATUS_MOVE);
        break;
    case EFFECT_GRASSY_TERRAIN:
