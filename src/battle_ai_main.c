    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        if (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_TOXIC_BOOST:
                if ((moveEffect == EFFECT_POISON || moveEffect == EFFECT_TOXIC) && HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL))
                    RETURN_SCORE_MINUS(20);
                break;
            case ABILITY_FLARE_BOOST:
                if (moveEffect == EFFECT_WILL_O_WISP && HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_SPECIAL))
                    RETURN_SCORE_MINUS(20);
                break;
            ...
            case ABILITY_FLOWER_VEIL:
                if (IsNonVolatileStatusMoveEffect(moveEffect) || IsStatLoweringEffect(moveEffect))
                    RETURN_SCORE_MINUS(10);
                break;
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_KNOCK_OFF:
            if (aiData->abilities[battlerDef] == ABILITY_STICKY_HOLD || aiData->abilities[battlerDef] == ABILITY_BALL_FETCH)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_INGRAIN:
        ...
        case EFFECT_INSTRUCT:
            {
                ...
                if (instructedMove == MOVE_NONE
                  ...
                  || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove)
                  || gMovesInfo[instructedMove].effect == EFFECT_RECHARGE_EXCEPT_FAINT)
                {
                    ADJUST_SCORE(-10);
                }
                else if (isDoubleBattle)
...
static u32 AI_CalcMoveScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    // check waking moves
    if ((gBattleMons[battlerAtk].status1 & STATUS1_SLEEP) && gMovesInfo[move].wakesUser)
        ADJUST_SCORE(10);
    ...
    case EFFECT_ION_DELUGE:
        if ((aiData->abilities[battlerAtk] == ABILITY_VOLT_ABSORB
          ...
          || aiData->abilities[battlerAtk] == ABILITY_MINUS)
            ...
            switch (gMovesInfo[move].additionalEffects[i].moveEffect)
            {
                ...
                case MOVE_EFFECT_BUG_BITE:   // And pluck
                    if (gBattleMons[battlerDef].status2 & STATUS2_SUBSTITUTE 
                        ...
                        || aiData->abilities[battlerDef] == ABILITY_BALL_FETCH)
                        break;
...
static s32 AI_HPAware(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         ...
         || (moveType == TYPE_ELECTRIC && AI_DATA->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_MINUS)
