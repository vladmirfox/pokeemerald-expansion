...
u8 GetBattlerYDelta(u8 battlerId, u16 species)
{
    ...
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER || IsContest())
    {
        if (species == SPECIES_CASTFORM)
            ...
            ret = gMonBackPicCoords[SanitizeSpeciesId(species)].y_offset;
    }
    else
    {
        if (species == SPECIES_CASTFORM)
            ...
            ret = gMonFrontPicCoords[SanitizeSpeciesId(species)].y_offset;
    }
    return ret;
}
...
s16 GetBattlerSpriteCoordAttr(u8 battlerId, u8 attr)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                coords = &gMonFrontPicCoords[SanitizeSpeciesId(species)];
            }
        }
    }
