...
u8 TrySetCantSelectMoveBattleScript(void)
{
    ...
    else if ((GetBattlerAbility(gActiveBattler) == ABILITY_MONKEY_BUSINESS) && *choicedMove != MOVE_NONE
              && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != move)
    {
        gCurrentMove = *choicedMove;
        gLastUsedItem = gBattleMons[gActiveBattler].item;
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Gorilla Tactics
            ...
            unusableMoves |= gBitTable[i];
        // Monkey Business
        else if (check & MOVE_LIMITATION_CHOICE_ITEM && GetBattlerAbility(battlerId) == ABILITY_MONKEY_BUSINESS && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != gBattleMons[battlerId].moves[i])
            unusableMoves |= gBitTable[i];
    }
...
u8 AtkCanceller_UnableToUseMove(u32 moveType)
{
    ...
    do
    {
        ...
        case CANCELLER_MULTIHIT_MOVES:
            ...
            else if (gBattleMoves[gCurrentMove].effect == EFFECT_TRIPLE_KICK || gBattleMoves[gCurrentMove].flags & FLAG_THREE_STRIKES || gCurrentMove == MOVE_TRI_ATTACK)
            {
                gMultiHitCounter = 3;
                PREPARE_BYTE_NUMBER_BUFFER(gBattleScripting.multihitString, 1, 0)
            }
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_EARTH_EATER:
                if(gStatuses3[gBattlerAttacker] & STATUS3_UNDERGROUND)  {  
                    BattleScriptPushCursorAndCallback(BattleScript_RainDishActivates);
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 4;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
            ...
            case ABILITY_SCHOOLING:
				if (!BATTLER_MAX_HP(battler) && !(gStatuses3[gActiveBattler] & STATUS3_HEAL_BLOCK) && gDisableStructs[battler].isFirstTurn != 2)
				{
                    ...
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    BattleScriptExecute(BattleScript_SchoolingRestoreActivates);
                    effect++;
                }
                break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_TANGLING_HAIR:
             ...
             && IsMoveMakingContact(move, gBattlerAttacker))
            {
                ...
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_GooeyActivates;
                gBattlescriptCurrInstr = BattleScript_TanglingHairTraps;
                gHitMarker |= HITMARKER_IGNORE_SAFEGUARD;
                effect++;
            }
            break;
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    case ABILITY_SAND_VEIL:
        if (IS_MOVE_PHYSICAL(move) && WEATHER_HAS_EFFECT && gBattleWeather & B_WEATHER_SANDSTORM)
            MulModifier(&modifier, UQ_4_12(1.2));
        break;
    case ABILITY_SNOW_CLOAK:
        if (IS_MOVE_SPECIAL(move) && WEATHER_HAS_EFFECT && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW)))
            MulModifier(&modifier, UQ_4_12(1.2));
        break;
    }
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    case ABILITY_IL_VATICANO:
        if (GetBattlerPosition(battlerAtk) == B_POSITION_PLAYER_LEFT && GetBattlerPosition(battlerDef) == B_POSITION_OPPONENT_RIGHT)
            MulModifier(&modifier, UQ_4_12(1.4));
        else if (GetBattlerPosition(battlerAtk) == B_POSITION_PLAYER_RIGHT && GetBattlerPosition(battlerDef) == B_POSITION_OPPONENT_LEFT)
            MulModifier(&modifier, UQ_4_12(1.4));
        break;
    }
