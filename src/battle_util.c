...
void HandleAction_UseMove(void)
{
    ...
    {
        gCurrentMove = gChosenMove = gLastPrintedMoves[gBattlerAttacker];
    }
...
#define R_HAS_MOVE_EFFECT(...) RECURSIVELY(R_FOR_EACH(HAS_MOVE_EFFECT, __VA_ARGS__))
#define HAS_MOVE_EFFECT(_condition) && effects[i]._condition
...
#define RETURN_HAS_MOVE_ADDITIONAL_EFFECT(condition1, ...)          \
    ...
    for (i = 0; i < count; i++)                                     \
    {                                                               \
        if (effects[i].condition1 R_HAS_MOVE_EFFECT(__VA_ARGS__))   \
            return TRUE;                                            \
    }                                                               \
    return FALSE;
