...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_ILLUSION:
            if (gBattleStruct->illusion[gBattlerTarget].on && !gBattleStruct->illusion[gBattlerTarget].broken && BattlerTurnDamaged(gBattlerTarget))
            {
                BattleScriptPushCursor();
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_POISON_TOUCH:
            if (MoveResultHasEffect(gBattlerTarget)
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker)
             && BattlerTurnDamaged(gBattlerTarget) // Need to actually hit the target
        ...
        case ABILITY_GULP_MISSILE:
            if ((gBattleMons[gBattlerAttacker].species == SPECIES_CRAMORANT)
             && ((gCurrentMove == MOVE_SURF && BattlerTurnDamaged(gBattlerTarget)) || gStatuses3[gBattlerAttacker] & STATUS3_UNDERWATER)
...
static u8 TrySetEnigmaBerry(u32 battler)
{
    if (IsBattlerAlive(battler)
     ...
     && ((BattlerTurnDamaged(battler) && gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_SUPER_EFFECTIVE) || gBattleScripting.overrideBerryRequirements)
...
u32 ItemBattleEffects(enum ItemEffect caseID, u32 battler, bool32 moveTurn)
{
    ...
    case ITEMEFFECT_TARGET:
        if (MoveResultHasEffect(gBattlerTarget))
        {
            ...
            {
            case HOLD_EFFECT_AIR_BALLOON:
                if (BattlerTurnDamaged(gBattlerTarget))
                {
                    effect = ITEM_EFFECT_OTHER;
                    BattleScriptPushCursor();
