        ...
        case ABILITY_EFFECT_SPORE:
        {
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_SAFETY_GOGGLES)
            {
                ...
                if (i < sleep
                 ...
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && IsBattlerTurnDamaged(gBattlerTarget)
                 && CanBeSleptOrDrowsy(gBattlerAttacker, ability, NOT_BLOCKED_BY_SLEEP_CLAUSE)
                 && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
                 && IsMoveMakingContact(move, gBattlerAttacker))
                {
                    if (IsSleepClauseEnabled())
...
u8 GetAttackerObedienceForAction()
{
    ...
    if (gBattleMons[gBattlerAttacker].status1 & (STATUS1_SLEEP | STATUS1_DROWSY) && (gMovesInfo[gCurrentMove].effect == EFFECT_SNORE || gMovesInfo[gCurrentMove].effect == EFFECT_SLEEP_TALK))
    ...
    else
    {
        ...
        calc = ((rnd >> 16) & 255);
        if (calc < obedienceLevel && CanBeSleptOrDrowsy(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), NOT_BLOCKED_BY_SLEEP_CLAUSE))
        {
            // try putting asleep
            int i;
            for (i = 0; i < gBattlersCount; i++)
...
static inline u32 CalcMoveBasePower(struct DamageCalculationData *damageCalcData, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_DOUBLE_POWER_ON_ARG_STATUS:
        ...
        if ((gBattleMons[battlerDef].status1 | ((STATUS1_SLEEP | STATUS1_DROWSY) * (abilityDef == ABILITY_COMATOSE))) & gMovesInfo[move].argument.status
