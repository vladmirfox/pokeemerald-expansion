...
bool32 MoveRecordedBattleToSaveData(void)
{
    ...
    battleSave->opponentA = sTrainerBattleParameter.params.battleOpponentA;
    battleSave->opponentB = sTrainerBattleParameter.params.battleOpponentB;
    ...
    if (sTrainerBattleParameter.params.battleOpponentA >= TRAINER_RECORD_MIXING_FRIEND && sTrainerBattleParameter.params.battleOpponentA < TRAINER_RECORD_MIXING_APPRENTICE)
    {
        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
            battleSave->recordMixFriendName[i] = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_FRIEND].name[i];
        battleSave->recordMixFriendClass = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_FRIEND].facilityClass;

        if (sBattleOutcome == B_OUTCOME_WON)
        {
            for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
                battleSave->easyChatSpeech[i] = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_FRIEND].speechLost[i];
        }
        else
        {
            for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
                battleSave->easyChatSpeech[i] = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_FRIEND].speechWon[i];
        }
        battleSave->recordMixFriendLanguage = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_FRIEND].language;
    }
    else if (sTrainerBattleParameter.params.battleOpponentB >= TRAINER_RECORD_MIXING_FRIEND && sTrainerBattleParameter.params.battleOpponentB < TRAINER_RECORD_MIXING_APPRENTICE)
    {
        for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
            battleSave->recordMixFriendName[i] = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_FRIEND].name[i];
        battleSave->recordMixFriendClass = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_FRIEND].facilityClass;

        if (sBattleOutcome == B_OUTCOME_WON)
        {
            for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
                battleSave->easyChatSpeech[i] = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_FRIEND].speechLost[i];
        }
        else
        {
            for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
                battleSave->easyChatSpeech[i] = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_FRIEND].speechWon[i];
        }
        battleSave->recordMixFriendLanguage = gSaveBlock2Ptr->frontier.towerRecords[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_FRIEND].language;
    }
    ...
    if (sTrainerBattleParameter.params.battleOpponentA >= TRAINER_RECORD_MIXING_APPRENTICE)
    {
        battleSave->apprenticeId = gSaveBlock2Ptr->apprentices[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_APPRENTICE].id;
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
            battleSave->easyChatSpeech[i] = gSaveBlock2Ptr->apprentices[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_APPRENTICE].speechWon[i];
        battleSave->apprenticeLanguage = gSaveBlock2Ptr->apprentices[sTrainerBattleParameter.params.battleOpponentA - TRAINER_RECORD_MIXING_APPRENTICE].language;
    }
    else if (sTrainerBattleParameter.params.battleOpponentB >= TRAINER_RECORD_MIXING_APPRENTICE)
    {
        battleSave->apprenticeId = gSaveBlock2Ptr->apprentices[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_APPRENTICE].id;
        for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
            battleSave->easyChatSpeech[i] = gSaveBlock2Ptr->apprentices[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_APPRENTICE].speechWon[i];
        battleSave->apprenticeLanguage = gSaveBlock2Ptr->apprentices[sTrainerBattleParameter.params.battleOpponentB - TRAINER_RECORD_MIXING_APPRENTICE].language;
    }
...
static void CB2_RecordedBattleEnd(void)
{
    ...
    sTrainerBattleParameter.params.battleOpponentB = 0;
    gPartnerTrainerId = 0;
...
void SetVariablesForRecordedBattle(struct RecordedBattleSave *src)
{
    ...
    sTrainerBattleParameter.params.battleOpponentA = src->opponentA;
    sTrainerBattleParameter.params.battleOpponentB = src->opponentB;
