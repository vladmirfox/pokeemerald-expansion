...
static bool8 MapHasSpecies(const struct WildPokemonHeader *info, u16 species)
{
    ...
    if (MonListHasSpecies(info->encounterTypes->landMonsInfo, species, LAND_WILD_COUNT))
        return TRUE;
    if (MonListHasSpecies(info->encounterTypes->waterMonsInfo, species, WATER_WILD_COUNT))
        return TRUE;
...
#ifdef BUGFIX
    if (MonListHasSpecies(info->encounterTypes->fishingMonsInfo, species, FISH_WILD_COUNT))
#else
    if (MonListHasSpecies(info->encounterTypes->fishingMonsInfo, species, LAND_WILD_COUNT))
#endif
        return TRUE;
    if (MonListHasSpecies(info->encounterTypes->rockSmashMonsInfo, species, ROCK_WILD_COUNT))
        return TRUE;
    return FALSE;
}
