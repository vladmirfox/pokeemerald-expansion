...
static bool32 ShouldFailForIllusion(u16 illusionSpecies, u32 battlerId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        for (j = 0; gSpeciesInfo[illusionSpecies].levelUpLearnset[j].move != MOVE_UNAVAILABLE; j++)
        {
            if (gSpeciesInfo[illusionSpecies].levelUpLearnset[j].move == move)
                break;
        }
        // The used move is in the learnsets of the fake species.
        if (gSpeciesInfo[illusionSpecies].levelUpLearnset[j].move != MOVE_UNAVAILABLE)
            continue;
